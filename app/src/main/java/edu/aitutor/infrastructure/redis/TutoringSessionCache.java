package edu.aitutor.infrastructure.redis;
...
public class TutoringSessionCache {
     ...
     * 缓存键前缀
     */
    private static final String SESSION_KEY_PREFIX = "aitutor:session:";

    /**
     * 简历ID到会话ID的映射前缀（用于查找未完成会话）
     */
    private static final String RESUME_SESSION_KEY_PREFIX = "aitutor:resume:";
    ...
    public static class CachedSession implements Serializable {
        ...
        private String studentProfileText;
        ...
        public CachedSession(String sessionId, String studentProfileText, Long studentProfileId,
                            List<TutoringQuestionDTO> questions, int currentIndex,
                            SessionStatus status, ObjectMapper objectMapper) {
            this.sessionId = sessionId;
            this.studentProfileText = studentProfileText;
        ...
        public List<TutoringQuestionDTO> getQuestions(ObjectMapper objectMapper) {
            try {
     ...
     * 保存会话到缓存
     */
    public void saveSession(String sessionId, String studentProfileText, Long studentProfileId,
                           List<TutoringQuestionDTO> questions, int currentIndex,
                           SessionStatus status) {
        String key = buildSessionKey(sessionId);
        CachedSession cachedSession = new CachedSession(
            sessionId, studentProfileText, studentProfileId, questions, currentIndex, status, objectMapper
        );

        redisService.set(key, cachedSession, SESSION_TTL);

        // 如果有 studentProfileId，建立映射关系（用于查找未完成会话）
        if (studentProfileId != null && isUnfinishedStatus(status)) {
            saveResumeSessionMapping(studentProfileId, sessionId);
        }

        log.debug("会话已缓存: sessionId={}, studentProfileId={}, status={}", sessionId, studentProfileId, status);
    }
    ...
    public void updateSessionStatus(String sessionId, SessionStatus status) {
        getSession(sessionId).ifPresent(session -> {
            ...
            // 如果会话已完成，移除映射
            if (!isUnfinishedStatus(status) && session.getStudentProfileId() != null) {
                removeStudentProfileSessionMapping(session.getStudentProfileId(), sessionId);
            }
     ...
     * 更新问题列表（用于保存答案）
     */
    public void updateQuestions(String sessionId, List<TutoringQuestionDTO> questions) {
        getSession(sessionId).ifPresent(session -> {
            try {
    ...
    public void deleteSession(String sessionId) {
        getSession(sessionId).ifPresent(session -> {
            if (session.getStudentProfileId() != null) {
                removeStudentProfileSessionMapping(session.getStudentProfileId(), sessionId);
            }
        });
     ...
     * 根据简历ID查找未完成的会话ID
     */
    public Optional<String> findUnfinishedSessionId(Long studentProfileId) {
        String key = buildResumeSessionKey(studentProfileId);
    ...
    private String buildResumeSessionKey(Long studentProfileId) {
        return RESUME_SESSION_KEY_PREFIX + studentProfileId;
    }

    private void saveResumeSessionMapping(Long studentProfileId, String sessionId) {
        String key = buildResumeSessionKey(studentProfileId);
        redisService.set(key, sessionId, SESSION_TTL);
    }

    private void removeStudentProfileSessionMapping(Long studentProfileId, String sessionId) {
        String key = buildResumeSessionKey(studentProfileId);
