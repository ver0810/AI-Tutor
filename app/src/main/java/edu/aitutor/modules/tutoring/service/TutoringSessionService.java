...
public class TutoringSessionService {
    ...
    private final TutoringQuestionService questionService;
     ...
     */
    public TutoringSessionDTO createSession(CreateTutoringRequest request) {
        // 如果指定了studentProfileId且未强制创建，检查是否有未完成的会话
        if (request.studentProfileId() != null && !Boolean.TRUE.equals(request.forceCreate())) {
            Optional<TutoringSessionDTO> unfinishedOpt = findUnfinishedSession(request.studentProfileId());
            if (unfinishedOpt.isPresent()) {
                log.info("检测到未完成的面试会话，返回现有会话: studentProfileId={}, sessionId={}",
                    request.studentProfileId(), unfinishedOpt.get().sessionId());
        ...
        log.info("创建新面试会话: {}, 题目数量: {}, studentProfileId: {}",
            sessionId, request.questionCount(), request.studentProfileId());

        // 获取历史问题
        List<String> historicalQuestions = null;
        if (request.studentProfileId() != null) {
            historicalQuestions = persistenceService.getHistoricalQuestionsByResumeId(request.studentProfileId());
        }

        // 生成面试问题
        List<TutoringQuestionDTO> questions = questionService.generateQuestions(
            request.studentProfileText(),
        ...
        if (request.studentProfileId() != null) {
            try {
                persistenceService.saveSession(sessionId, request.studentProfileId(),
     ...
     */
    public TutoringSessionDTO getSession(String sessionId) {
        // 1. 尝试从 Redis 缓存获取
     ...
     * 查找并恢复未完成的面试会话
     */
    public Optional<TutoringSessionDTO> findUnfinishedSession(Long studentProfileId) {
        try {
            // 1. 先从 Redis 缓存查找
            Optional<String> cachedSessionIdOpt = sessionCache.findUnfinishedSessionId(studentProfileId);
            if (cachedSessionIdOpt.isPresent()) {
                String sessionId = cachedSessionIdOpt.get();
                Optional<CachedSession> cachedOpt = sessionCache.getSession(sessionId);
                if (cachedOpt.isPresent()) {
                    log.debug("从 Redis 缓存找到未完成会话: studentProfileId={}, sessionId={}", studentProfileId, sessionId);
                    return Optional.of(toDTO(cachedOpt.get()));
                }
            }

            // 2. 缓存未命中，从数据库查找
            Optional<TutoringSessionEntity> entityOpt = persistenceService.findUnfinishedSession(studentProfileId);
            if (entityOpt.isEmpty()) {
                return Optional.empty();
            }

            TutoringSessionEntity entity = entityOpt.get();
     ...
     */
    public TutoringSessionDTO findUnfinishedSessionOrThrow(Long studentProfileId) {
        return findUnfinishedSession(studentProfileId)
            .orElseThrow(() -> new BusinessException(ErrorCode.TUTORING_SESSION_NOT_FOUND, "未找到未完成的面试会话"));
    }
    ...
    private CachedSession restoreSessionFromDatabase(String sessionId) {
        try {
            Optional<TutoringSessionEntity> entityOpt = persistenceService.findBySessionId(sessionId);
     ...
     * 从实体恢复会话并缓存到 Redis
     */
    private CachedSession restoreSessionFromEntity(TutoringSessionEntity entity) {
        try {
            // 解析问题列表
            ...
            // 恢复已保存的答案
            List<TutoringAnswerEntity> answers = persistenceService.findAnswersBySessionId(entity.getSessionId());
    ...
    private SessionStatus convertStatus(TutoringSessionEntity.SessionStatus status) {
    ...
    public Map<String, Object> getCurrentQuestionResponse(String sessionId) {
        TutoringQuestionDTO question = getCurrentQuestion(sessionId);
        if (question == null) {
            return Map.of(
                "completed", true,
                "message", "所有问题已回答完毕"
     ...
     * 获取当前问题
     */
    public TutoringQuestionDTO getCurrentQuestion(String sessionId) {
        CachedSession session = getOrRestoreSession(sessionId);
        List<TutoringQuestionDTO> questions = session.getQuestions(objectMapper);
    ...
    public SubmitAnswerResponse submitAnswer(SubmitAnswerRequest request) {
        ...
        if (index < 0 || index >= questions.size()) {
            throw new BusinessException(ErrorCode.TUTORING_QUESTION_NOT_FOUND, "无效的问题索引: " + index);
        }

        // 更新问题答案
        TutoringQuestionDTO question = questions.get(index);
        TutoringQuestionDTO answeredQuestion = question.withAnswer(request.answer());
        ...
        TutoringQuestionDTO nextQuestion = hasNextQuestion ? questions.get(newIndex) : null;
        ...
        try {
            ...
            persistenceService.updateSessionStatus(request.sessionId(),
                newStatus == SessionStatus.COMPLETED
                    ? TutoringSessionEntity.SessionStatus.COMPLETED
                    : TutoringSessionEntity.SessionStatus.IN_PROGRESS);
     ...
     * 提前交卷（触发异步评估）
     */
    public void completeTutoring(String sessionId) {
        CachedSession session = getOrRestoreSession(sessionId);

        if (session.getStatus() == SessionStatus.COMPLETED || session.getStatus() == SessionStatus.EVALUATED) {
            throw new BusinessException(ErrorCode.TUTORING_ALREADY_COMPLETED);
        }
     ...
     * 生成评估报告
     */
    public TutoringReportDTO generateReport(String sessionId) {
        CachedSession session = getOrRestoreSession(sessionId);

        if (session.getStatus() != SessionStatus.COMPLETED && session.getStatus() != SessionStatus.EVALUATED) {
            throw new BusinessException(ErrorCode.TUTORING_NOT_COMPLETED, "辅导会话尚未完成，无法生成报告");
        }

        log.info("生成辅导报告: {}", sessionId);

        List<TutoringQuestionDTO> questions = session.getQuestions(objectMapper);

        TutoringReportDTO report = evaluationService.evaluateTutoring(
            sessionId,
            session.getStudentProfileText(),
            questions
        );
     ...
     * 将缓存会话转换为 DTO
     */
    private TutoringSessionDTO toDTO(CachedSession session) {
        List<TutoringQuestionDTO> questions = session.getQuestions(objectMapper);
        return new TutoringSessionDTO(
