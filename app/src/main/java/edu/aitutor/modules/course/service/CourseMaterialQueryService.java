...
public class CourseMaterialQueryService {
    ...
    private final CourseMaterialCountService countService;
    private final PromptTemplate systemPromptTemplate;
    private final PromptTemplate userPromptTemplate;

    public CourseMaterialQueryService(
            ChatClient.Builder chatClientBuilder,
            CourseMaterialVectorService vectorService,
            CourseMaterialListService listService,
            CourseMaterialCountService countService,
            @Value("classpath:prompts/course.model-query-system.st") Resource systemPromptResource,
            @Value("classpath:prompts/course.model-query-user.st") Resource userPromptResource) throws IOException {
    ...
    public String answerQuestion(List<Long> knowledgeBaseIds, String question) {
        ...
        // 2. 使用向量搜索检索相关文档（RAG）
        // 从知识库中获取 courseId 用于数据隔离
        Long courseId = listService.getPrimaryCourseId(knowledgeBaseIds);
        List<Document> relevantDocs = vectorService.similaritySearch(question, knowledgeBaseIds, courseId, 5);
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.COURSE_MATERIAL_QUERY_FAILED, "知识库查询失败：" + e.getMessage());
        }
    }
     ...
     * 查询知识库并返回完整响应
     */
    public QueryResponse queryCourseMaterial(QueryRequest request) {
        String answer = answerQuestion(request.courseMaterialIds(), request.question());

        // 获取知识库名称（多个知识库用逗号分隔）
        List<String> kbNames = listService.getCourseMaterialNames(request.courseMaterialIds());
        String kbNamesStr = String.join("、", kbNames);

        // 使用第一个知识库ID作为主要标识（兼容前端）
        Long primaryKbId = request.courseMaterialIds().getFirst();
