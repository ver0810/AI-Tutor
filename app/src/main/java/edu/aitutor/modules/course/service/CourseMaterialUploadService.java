...
public class CourseMaterialUploadService {

    private final CourseMaterialParseService parseService;
    private final CourseMaterialPersistenceService persistenceService;
     ...
     */
    public Map<String, Object> uploadCourseMaterial(MultipartFile file, Long courseId, String name, String category) {
        // 1. 验证文件
        ...
        Optional<CourseMaterialEntity> existingKb = knowledgeBaseRepository.findByFileHash(fileHash);
        if (existingKb.isPresent()) {
            log.info("检测到重复知识库: hash={}", fileHash);
            return persistenceService.handleDuplicateCourseMaterial(existingKb.get(), fileHash);
        }
        ...
        // 5. 保存文件到RustFS
        String fileKey = storageService.uploadCourseMaterial(file);
        ...
        return Map.of(
            "knowledgeBase", Map.of(
                ...
                "vectorStatus", MaterialVectorStatus.PENDING.name()
            ),
            "storage", Map.of(
                "fileKey", fileKey,
                "fileUrl", fileUrl
    ...
    private void validateContentType(String contentType, String fileName) {
        fileValidationService.validateContentType(
            contentType,
            fileName,
            fileValidationService::isCourseMaterialMimeType,
    ...
    public void revectorize(Long kbId) {
        CourseMaterialEntity kb = knowledgeBaseRepository.findById(kbId)
