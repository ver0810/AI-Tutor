 ...
 * 测验会话实体 (业务重构: 面试 → 测验)
 * 
 * 映射: 面试会话(interview session) → 测验会话(quiz session)
 * 关联: resume → courseMaterial (课程资料)
 * 状态: CREATED/IN_PROGRESS → COMPLETED → EVALUATED
 */
@Entity
@Table(name = "interview_sessions", indexes = {
    @Index(name = "idx_interview_session_id", columnList = "sessionId", unique = true),
    @Index(name = "idx_interview_resume", columnList = "resumeId"),
    @Index(name = "idx_interview_course", columnList = "courseId")
})
