...
import type {TutoringItem} from '../api/history';
...
interface TutoringPanelProps {
  ...
  onViewTutoring: (sessionId: string) => void;
  onExportTutoring: (sessionId: string) => void;
  onDeleteTutoring: (sessionId: string) => void;
  exporting: string | null;
  loadingTutoring: boolean;
}
...
export default function TutoringPanel({
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await historyApi.deleteTutoring(sessionId);
      onDeleteTutoring(sessionId);
  ...
  const chartData = useMemo(() => {
    return tutorings
      ...
      .map((tutoring) => ({
        name: formatDateOnly(tutoring.createdAt),
        score: tutoring.overallScore || 0,
        index: tutorings.length - tutorings.indexOf(tutoring)
      }))
      .reverse();
  }, [tutorings]);
  ...
  if (tutorings.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-12 text-center">
        ...
        <p className="text-slate-500 mb-6">开始课后测验，获取 AI 助教评估</p>
        <motion.button
  ...
  return (
    <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-slate-800">测验表现趋势</span>
            </div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <span className="font-semibold text-slate-800">历史测验记录</span>
        </div>
        ...
        <div className="space-y-4">
          {tutorings.map((tutoring, index) => (
            <TutoringItemCard
              ...
              exporting={exporting === tutoring.sessionId}
              deleting={deletingSessionId === tutoring.sessionId}
              onView={() => onViewTutoring(tutoring.sessionId)}
              onExport={() => onExportTutoring(tutoring.sessionId)}
              onDelete={(e) => handleDeleteClick(tutoring.sessionId, e)}
            />
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg ${
        tutoring.overallScore !== null 
          ? getScoreColor(tutoring.overallScore, [85, 70])
          : 'bg-slate-100 text-slate-400'
      }`}>
        {tutoring.overallScore ?? '-'}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 text-sm text-slate-500">
          <span className="flex items-center gap-1">
            ...
            {formatDateOnly(tutoring.createdAt)}
          </span>
          <span className="flex items-center gap-1">
