...
import type {TutoringDetail} from '../api/history';

interface TutoringDetailPanelProps {
...
export default function TutoringDetailPanel({ tutoring }: TutoringDetailPanelProps) {
  ...
  const [expandedQuestions, setExpandedQuestions] = useState<Set<number>>(() => {
    ...
    if (tutoring.answers) {
      tutoring.answers.forEach((_, idx) => allIndices.add(idx));
    }
    return allIndices;
  });
  ...
  const { scorePercent, circumference, strokeDashoffset } = useMemo(() => {
    const percent = tutoring.overallScore !== null ? (tutoring.overallScore / 100) * 100 : 0;
  ...
  return (
    ...
    >
      {/* 评分卡片 */}
      <ScoreCard 
        score={tutoring.overallScore}
        feedback={tutoring.overallFeedback}
      ...
      {/* 表现优势 */}
      {tutoring.strengths && tutoring.strengths.length > 0 && (
        <StrengthsSection strengths={tutoring.strengths} />
      )}

      {/* 改进建议 */}
      {tutoring.improvements && tutoring.improvements.length > 0 && (
        <ImprovementsSection improvements={tutoring.improvements} />
      )}

      {/* 问答记录详情 */}
      <QuestionsSection 
        answers={tutoring.answers || []}
