...
export default function ResumeDetailPage({ resumeId, onBack, onStartInterview }: ResumeDetailPageProps) {
  ...
  const handleExportAnalysisPdf = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `课程资料分析报告_${resume?.filename || resumeId}.pdf`;
  ...
  const tabs = [
    ...
    { id: 'interview' as const, label: '测验记录', icon: MessageSquare, count: resume.interviews?.length || 0 },
  ];
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">
              {detailView === 'interviewDetail' ? `测验详情 #${selectedInterview?.sessionId?.slice(-6) || ''}` : resume.filename}
            </h2>
