...
interface CourseMaterialManagePageProps {
  onUpload: () => void;
  onChat: () => void;
}
...
export default function CourseMaterialManagePage({ onUpload, onChat }: CourseMaterialManagePageProps) {
  const [stats, setStats] = useState<CourseMaterialStats | null>(null);
  const [knowledgeBases, setCourseMaterials] = useState<CourseMaterialItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchKeyword, setSearchKeyword] = useState('');
  const [sortBy, setSortBy] = useState<SortOption>('time');
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [categories, setCategories] = useState<string[]>([]);
  const [deleteItem, setDeleteItem] = useState<CourseMaterialItem | null>(null);
  ...
  const loadDataSilent = useCallback(async () => {
    try {
      const [statsData, kbList, categoryList] = await Promise.all([
        courseMaterialApi.getStatistics(),
        searchKeyword
          ? courseMaterialApi.search(searchKeyword)
          : selectedCategory
          ? courseMaterialApi.getByCategory(selectedCategory)
          : courseMaterialApi.getAllCourseMaterials(sortBy),
        courseMaterialApi.getAllCategories(),
      ]);
      setStats(statsData);
      setCourseMaterials(kbList);
  ...
  const handleRevectorize = async (id: number) => {
    try {
      setRevectorizing(id);
      await courseMaterialApi.revectorize(id);
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await courseMaterialApi.deleteCourseMaterial(deleteItem.id);
      setDeleteItem(null);
      await loadData();
    } catch (error) {
  ...
  // 下载课程资料
  const handleDownload = (kb: CourseMaterialItem) => {
    // 通过后端 API 下载文件
    const link = document.createElement('a');
    link.href = `/api/courseMaterial/${kb.id}/download`;
  ...
  // 开始编辑分类
  const handleStartEditCategory = (kb: CourseMaterialItem) => {
  ...
  const handleSaveCategory = async (id: number) => {
    try {
      ...
      await courseMaterialApi.updateCategory(id, categoryToSave);
      setEditingCategoryId(null);
      setEditingCategoryValue('');
      await loadData();
    } catch (error) {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">管理您的课程资料文件，查看使用统计</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-slate-500">暂无课程资料</p>
            <button
              onClick={onUpload}
